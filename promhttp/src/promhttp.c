...
typedef enum MHD_Result AccessHandlerResult;
#else
typedef int AccessHandlerResult;
#endif
...
static AccessHandlerResult promhttp_handler(void *cls, struct MHD_Connection *connection, const char *url, const char *method,
  ...
  if (strcmp(method, "GET") != 0) {
    ...
    AccessHandlerResult ret = MHD_queue_response(connection, MHD_HTTP_BAD_REQUEST, response);
  ...
  if (strcmp(url, "/") == 0) {
    char *buf = "OK\n";
    struct MHD_Response *response = MHD_create_response_from_buffer(strlen(buf), (void *)buf, MHD_RESPMEM_PERSISTENT);
    AccessHandlerResult ret = MHD_queue_response(connection, MHD_HTTP_OK, response);
    MHD_destroy_response(response);
    return ret;
  }
